        ...
        match event {
            Event::MenuEvent {
                ...
                if menu_id == quit.clone().id() {
                    let _ = tx.blocking_send(EventTriggers::Stop);
                }

                if menu_id == configure.clone().id() {
                    let _ = tx.blocking_send(EventTriggers::OpenUi);
                }

                if menu_id == profiles.clone().id() {
                    let _ = tx.blocking_send(EventTriggers::Open(Profiles));
                }

                if menu_id == mic_profiles.clone().id() {
                    let _ = tx.blocking_send(EventTriggers::Open(MicProfiles));
                }

                if menu_id == presets.clone().id() {
                    let _ = tx.blocking_send(EventTriggers::Open(Presets));
                }

                if menu_id == samples.clone().id() {
                    let _ = tx.blocking_send(EventTriggers::Open(Samples));
                }

                if menu_id == icons.clone().id() {
                    let _ = tx.blocking_send(EventTriggers::Open(Icons));
                }
            }
    ...
    fn window_proc(
        ...
        match msg {
            ...
            winapi::um::winuser::WM_ENDSESSION => {
                ...
                let _ = self.global_tx.blocking_send(EventTriggers::Stop);
