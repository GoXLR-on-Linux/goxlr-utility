...
impl GenderEncoderBase {
    ...
    pub fn parse_gender_preset(&mut self, id: u8, attributes: &Vec<OwnedAttribute>) {
        ...
        for attr in attributes {
            ...
            println!("[GenderEncoder] Unparsed Child Attribute: {}", &attr.name.local_name);
        }
    ...
    pub fn write_gender(&self, writer: &mut EventWriter<&mut File>) {
        ...
        for (key, value) in &self.preset_map {
            ...
            sub_attributes.insert("GENDER_KNOB_POSITION".to_string(), format!("{}", value.knob_position));
            sub_attributes.insert("GENDER_STYLE".to_string(), value.style.get_str("uiIndex").unwrap().to_string());
