...
pub enum ParseError {
    #[error("[Pitch] Expected int: {0}")]
    ExpectedInt(#[from] std::num::ParseIntError),

    #[error("[Pitch] Expected float: {0}")]
    ExpectedFloat(#[from] std::num::ParseFloatError),

    #[error("[Pitch] Expected enum: {0}")]
    ExpectedEnum(#[from] strum::ParseError),

    #[error("[Pitch] Invalid colours: {0}")]
...
impl PitchEncoderBase {
    ...
    pub fn parse_pitch_root(&mut self, attributes: &Vec<Attribute>) -> Result<(), ParseError> {
